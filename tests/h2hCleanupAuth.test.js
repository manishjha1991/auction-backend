...
const protectedAdminPaths = [
  [headToHeadRoutes, 'post', '/reset'],
  [headToHeadRoutes, 'post', '/sync'],
  [userRoutes, 'post', '/cleanup-bought-players'],
];
...
test('head-to-head reset/sync and cleanup-bought-players require JWT admin middleware', () => {
  for (const [router, method, path] of protectedAdminPaths) {
...
test('unauthenticated callers cannot reach head-to-head reset or cleanup-bought-players', async () => {
...
test('forged JWT without matching server session cannot authorize head-to-head reset', async () => {
  const stack = getRouteStack(headToHeadRoutes, 'post', '/reset');
  const authHandler = stack[0].handle;
